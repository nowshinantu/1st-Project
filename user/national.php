...
<?php
    session_start();
    $electioncode = "SELECT Max(ElectionCode),Votetype FROM pratice INNER JOIN votingcode ON pratice.NationalCode = votingcode.AreaCode  INNER JOIN electioncode ON electioncode.AreaCode = votingcode.VotingCode WHERE email = '".$_SESSION["email"]."'";
...
<button> <i class="fa fa-power-off"></i><a href="signout.php"> Log Out</a></button>
    ...
    <div class="container">
       <form action="server/nationalcast.php" method="POST">
       <div class="column">
           <div class="title-header">
               <h3>Member of Parliament</h3>
               <?php
               $mayorsql = "SELECT * FROM symbols INNER JOIN candidate_registration ON candidate_registration.CandidateCode = symbols.CandidateCode WHERE candidate_registration.NominationFor ='Member of Parliament' AND symbols.ElectionCode = '".$row["ElectionCode"]."'";
    ...
    echo "invalid info about your national election. ";
}
